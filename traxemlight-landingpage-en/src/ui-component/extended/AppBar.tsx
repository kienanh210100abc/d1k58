...
const AppBar = ({ ...others }) => {
    ...
    console.log('currentLanguage', onChangeLocaleRef);
    ...
    useEffect(() => {
        ...
        if (savedLanguage) {
            setCurrentLanguage(savedLanguage as 'EN' | 'VN');
        } else {
    ...
    useEffect(() => {
        const updateLanguage = (lang: 'EN' | 'VN') => {
            setCurrentLanguage(lang);
            onChangeLocaleRef.current(lang);
            dispatch(setLanguage(lang));
        };

        // Kiểm tra URL và cập nhật ngôn ngữ
        if (location.pathname.startsWith('/en')) {
            updateLanguage('EN');
            localStorage.setItem('selectedLanguage', 'EN');
        } else {
            // Nếu không phải '/en', chuyển hướng về '/en'
    ...
    const handleClose = (language: 'EN' | 'VN') => {
        ...
        if (language === 'VN') {
            // Chuyển hướng đến trang tiếng Việt
            window.location.href = window.location.origin + location.pathname.replace(/^\/en/, '');
        } else if (language === 'EN') {
            // Đảm bảo URL bắt đầu bằng '/en'
            if (!location.pathname.startsWith('/en')) {
                const newPath = '/en' + (location.pathname === '/' ? '' : location.pathname);
